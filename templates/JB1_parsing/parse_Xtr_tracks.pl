...
{
        local $/ = undef;
        open TL, "<Xtr_trackList.json" or die "couldn't open combined.json: $!";
        $blob = <TL>;
        close TL;
}
...
for my $id (keys %tracks) {
    ...
    if ($tracks{$id}{'urlTemplate'} =~ /\.\./) {
         $tracks{$id}{'urlTemplate'} =~ s/\.\.\/\.\./\/XenBaseData/;
    } elsif ($tracks{$id}{'urlTemplate'} !~ /^https/) {
         $tracks{$id}{'urlTemplate'} = '/XenBaseXTJBrowse/'.$tracks{$id}{'urlTemplate'};
    }
      ...
      "displays": [
        {
          ...
          "displayId": "XTR_$id-LinearVariantDisplay"
        },
        {
          "type": "ChordVariantDisplay",
          "displayId": "XTR_$id-ChordVariantDisplay"
        }
      ]
    },
|;
      ...
      "displays": [
        {
          ...
          "displayId": "XTR_$id-LinearBasicDisplay"
        },
        {
          "type": "LinearArcDisplay",
          "displayId": "XTR_$id-LinearArcDisplay"
        }
      ]
    },
|;
